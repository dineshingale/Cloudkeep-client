...
export default function ThoughtCard({
  ...
  return (
    <div 
      ...
      className={`w-full bg-white rounded-xl shadow-sm border overflow-hidden transition-all duration-300 ${
        isEditing ? 'border-blue-400 ring-4 ring-blue-50' : 'border-slate-200 hover:shadow-md'
      }`}
    >
      
      {/* --- HEADER --- */}
      <div className={`p-4 border-b flex items-center justify-between ${
        isEditing ? 'bg-blue-50/50 border-blue-100' : 'bg-white border-slate-50'
      }`}>
      ...
      <div className={`p-5 flex flex-col gap-4 ${
        isEditing ? 'bg-slate-50 min-h-[150px]' : 'bg-slate-50/10'
      }`}>
